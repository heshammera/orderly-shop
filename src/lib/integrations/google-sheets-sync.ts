...
export async function syncOrderToGoogleSheets(orderId: string, storeId: string): Promise<{ success: boolean; results: SyncResult[]; message?: string }> {
    try {
        ...
        if (orderError || !order) {
            ...
            return { success: false, message: 'Order not found', results: [] };
        }
        ...
        if (storeError || !store) {
            return { success: false, message: 'Store not found', results: [] };
        }
        ...
        if (integrationsError) {
            ...
            return { success: false, message: 'Failed to fetch integrations', results: [] };
        }

        if (!integrations || integrations.length === 0) {
            return { success: true, message: 'No active Google Sheet integrations', results: [] };
        }
        ...
        if (hasSuccess) {
            await supabase.from('orders').update({ is_synced: true }).eq('id', orderId);
        }

        return { success: true, results };
    } catch (error: any) {
        console.error('Error syncing to Google Sheets:', error);
        return { success: false, message: error.message || 'Internal Error', results: [] };
    }
}

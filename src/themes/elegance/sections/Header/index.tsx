...
export default function Header({ settings, blocks = [], sectionId = 'header_1', storeContext }: HeaderProps) {
    ...
    return (
        <header className="bg-slate-900 sticky top-0 z-50">
            ...
            <div className="container mx-auto flex justify-between items-center py-4 px-4 h-20 md:h-28">
                {/* Left section: Logo */}
                <Link href={storeSlug ? '' : '/'} className="flex items-center shrink-0">
                    {settings.logo || store?.logo_url ? (
                        <img src={settings.logo || store?.logo_url} alt={storeName} className="h-12 md:h-20 w-auto mr-4 object-contain" />
                    ) : (
